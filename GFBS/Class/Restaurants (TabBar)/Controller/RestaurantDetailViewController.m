...
-(void)setUpTitleView
{
    ...
    NSArray *titleContens = @[ZBLocalized(@"Overview", nil), ZBLocalized(@"Review", nil), ZBLocalized(@"Photo", nil), ZBLocalized(@"Menu", nil), ZBLocalized(@"Event", nil)];
...
-(void)loadCheckins
{
    ...
    [[GFHTTPSessionManager shareManager] POSTWithURLString:GetURL parameters:parameters success:^(id data) {
        ...
        NSLog(@"self.contents[0] %@", self.contents[0]);
